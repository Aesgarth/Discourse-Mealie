...
module Jobs
  class SyncMealieRecipes < ::Jobs::Scheduled
    ...
    def execute(args)
      ...
      return if SiteSetting.mealie_sync_frequency == :never
      
      # Hourly synchronization
      if SiteSetting.mealie_sync_frequency == :hourly
        sync_recipes
      # Daily synchronization - only run once per day
      elsif SiteSetting.mealie_sync_frequency == :daily && Time.now.hour == 0
        sync_recipes
      end
    end
...
# app/jobs/regular/import_recipe.rb
module Jobs
